...
#ifdef PU_CHA
    PUlist influencePUlist;

    int channelState;// channel state of current channel
...

{
  ...
  public:
    ...
    /** Called from ChannelAccess and Radio, to check whether the next hop is in state OFF, if is OFF, then send packet and update buffer.  Di Li   **/
    virtual bool checkNextHop_Channel_State(RadioRef RadioRef, AirFrame * airFrame);
...
#ifdef PU_CHA
    void addPU(PrimaryUserRef* PU);
    void allocatePU(RadioRef radio, Coord pos);
    void displayPUlist();
    void displayInfluencingPU(RadioRef radio);
    PUlist getPU(Coord RadioPos);
#ifdef MULTI_CHA
    void displayChannelTable(RadioRef r);
#endif
