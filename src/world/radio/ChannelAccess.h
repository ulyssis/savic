...
class AirFrame;
#ifdef PU_CHA
typedef std::map<PrimaryUser*, int> PUStateList;
#endif
#ifdef CAVCAPBUFFER
typedef std::map<AirFrame*, simtime_t> Buffer;
#endif
/*#ifdef MULTI_CHA
typedef std::map<Coord*, int> PUPositionChannelTable;
#endif*/
//#define BUFFER_OUTOFDATE 20
//BUFFER_OUTOFDATE is 2/10 before
//#define CHECKBUFFER_INTERVAL 1
/**
...
#ifdef PU_CHA
  public:
    static simsignal_t PUStateChangedSignal;
    PUStateList puStateList ;
//    IPv4Address myAddr;
#ifdef MULTI_CHA
    static simsignal_t NeighborChannelSwitchSignal;

//    PUPositionChannelTable puChannelTable;
  //  int numChannel;
   // int availableChannel [5];

#endif
#endif
#ifdef CAVCAPBUFFER
    Buffer buffer;
    cMessage* CheckBufferMsg;
    int schedulecounter;
    double CHECKBUFFER_INTERVAL;
    double BUFFER_OUTOFDATE;
    // cMessage* BufferTimeoutMsg;
#endif
  ...
  public:
    ...
    bool isInfluencedPU(PrimaryUser* pu);
#ifdef MULTI_CHA
 // void notificatonNeighborNewChannel(IPv4Address, int);
 /*   void displayPUPositionChannelTable(PUPositionChannelTable table);
    void displayAvailableChannel(int []);
    void channelselection(PUPositionChannelTable table, long whosepu);
    int getNewChannel(int oldchannel);
*/
#endif
#endif
