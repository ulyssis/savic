...
#ifdef PU_CHA
    typedef std::list<PrimaryUserRef> PUlist;
#endif
...
class INET_API IChannelControl
{
  ...
  public:
    ...
    /** Called from ChannelAccess and Radio, to check whether the next hop is in state OFF, if is OFF, then send packet or update buffer.  Di Li   **/
    virtual bool checkNextHop_Channel_State(RadioRef RadioRef, AirFrame * airFrame) = 0;
    ...
    virtual void addPU(PrimaryUserRef* PU)=0;
    virtual void allocatePU(RadioRef radio, Coord pos)=0;

    virtual PUlist getPU(Coord RadioPos)=0;
