...
void ARP::processOutboundPacket(cMessage *msg)
{
	...
	double txp = controlInfo->getTransmitterPower();
            ...
            nextHopAddr == ie->ipv4Data()->getIPAddress().getBroadcastAddress(ie->ipv4Data()->getNetmask())) // also include the network broadcast
    {
        ...
        sendPacketToNIC(msg, ie, MACAddress::BROADCAST_ADDRESS, ETHERTYPE_IPv4, txp);
        return;
    }
    ...
    if (nextHopAddr.isMulticast())
    {
        ...
        sendPacketToNIC(msg, ie, macAddr, ETHERTYPE_IPv4, txp);
        return;
    }
    ...
    if (globalARP)
    {
        ...
        sendPacketToNIC(msg, ie, (*it).second->macAddress, ETHERTYPE_IPv4, txp);
        return;
    }
...
void ARP::sendPacketToNIC(cMessage *msg, InterfaceEntry *ie, const MACAddress& macAddress, int etherType, double txp)
{
    // add control info with MAC address
    Ieee802Ctrl *controlInfo = new Ieee802Ctrl();
    controlInfo->setDest(macAddress);
    controlInfo->setEtherType(etherType);
	controlInfo->setTransmitterPower(txp);
...
void ARP::sendARPRequest(InterfaceEntry *ie, IPv4Address ipAddress)
{
    ...
    sendPacketToNIC(arp, ie, broadcastAddress, ETHERTYPE_ARP, -1);
    numRequestsSent++;
    ...
    if (addressRecognized(arp->getDestIPAddress(), ie))
    {
        ...
        switch (arp->getOpcode())
        {
            case ARP_REQUEST:
            {
                ...
                sendPacketToNIC(arp, ie, srcMACAddress, ETHERTYPE_ARP, -1);
                numRepliesSent++;
...
void ARP::updateARPCache(ARPCacheEntry *entry, const MACAddress& macAddress)
{
    ...
    while (!pendingPackets.empty())
    {
        ...
        sendPacketToNIC(msg, entry->ie, macAddress, ETHERTYPE_IPv4, -1);
    }
}
