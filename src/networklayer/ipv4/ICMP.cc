...
void ICMP::sendErrorMessage(IPv4Datagram *origDatagram, ICMPType type, ICMPCode code)
{
    ...
    std::cout<< "sending ICMP error " << msgname << endl;
    ...
    if (origDatagram->getSrcAddress().isUnspecified())
    {
       ...
       std::cout<<"then process it locally"<<endl;
        processICMPMessage(errorMessage);
    }
    else
    {   std::cout<<"sendToIp"<<endl;
...
{   std::cout<<"ICMP::processICMPMessage, get type: "<<icmpmsg->getType()<<endl;
