...
void UDP::handleMessage(cMessage *msg)
{
    ...
    if (msg->arrivedOn("ipIn") || msg->arrivedOn("ipv6In"))
    {
        ...
        if (IPv4Datagram * datagram=dynamic_cast<IPv4Datagram *>(msg))
        {

            IPv4ControlInfo * controlinfo=check_and_cast<IPv4ControlInfo *>(datagram->removeControlInfo());
            datagram->setControlInfo(controlinfo);
           // std::cout<<"in udp, check control info class, "<< controlinfo->getClassName()<<endl;
            UDPPacket * udppkt=dynamic_cast<UDPPacket *>(datagram->decapsulate());
            udppkt->setControlInfo(controlinfo);
            processUDPPacket(udppkt);
        }
        else
...
void UDP::processUndeliverablePacket(UDPPacket *udpPacket, cObject *ctrl)
{
    ...
    if (dynamic_cast<IPv4ControlInfo *>(ctrl) != NULL)
    {
        ...
        if (!ctrl4->getDestAddr().isMulticast())
        {
            ...
            //std::cout<<"UDP::processUndeliverPacket, ICMP sendErrorMessage"<<endl;
        }
        else
#endif
            delete udpPacket;
...
{   //std::cout<<"UDP::sendDown, srcIP:"<<srcAddr<<", destIP: "<<destAddr<<endl;
