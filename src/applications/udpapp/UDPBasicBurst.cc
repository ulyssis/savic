...
{   std::cout<<"UDPBasicBurst::handleMessage"<<endl;
    if (msg->isSelfMessage())
    {   std::cout<<"UDPBasicBurst::handleMessage, is selfmsg"<<endl;
    ...
    else if (msg->getKind() == UDP_I_DATA)
    {
        ...
        std::cout<<"UDPBasicBurst::handleMessage, processPacket"<<endl;
    }
    else if (msg->getKind() == UDP_I_ERROR)
    {
        EV << "Ignoring UDP error report\n";
        std::cout << "Ignoring UDP error report\n";
        delete msg;
    }
    else
    {
        error("Unrecognized message (%s)%s", msg->getClassName(), msg->getName());
        std::cout<<"UDPBasicBurst::handleMessage, error"<<endl;
    }

    if (ev.isGUI())
    {   std::cout<<"from isGUI"<<endl;
        char buf[40];
...
void UDPBasicBurst::generateBurst()
{
    ...
    if (activeBurst && nextBurst <= now) // new burst
    {
        ...
        else
        {
            ...
            std::cout<<"UDPBasicBurst::generateBurst, nextburst:"<<nextBurst<<", nextSleep:"<<nextSleep<<endl;
        }
